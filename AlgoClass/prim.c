...
void MST_PRIM(root* list, tHeap *PQ, int vertex_number, int *d, int *p, int* flag);
...
int main(void) {
	...
	for (i = 0; i < edge_number; i++) {
		...
		initList(list, temp2, end, weight);
	}
	...
	MST_PRIM(list, PQ, vertex_number, d, p, flag);
	...
	while(heapsize != 0){
		...
		if (flag[u] == 0) {
			for (temp = list[u].first_node; temp; temp = temp->next) {
				if (flag[temp->vertex] == 0 && temp->weight < d[temp->vertex]) {
					p[temp->vertex] = u;
